...
import { sendWhatsapp } from '../../redux/whatsapp'
import { useStateData } from '../../hooks'

export const MktViewCertificate = () => {
  const { data, changeData, cleanData } = useStateData({})
  const whatsappState = useReduxState('whatsapp')
  const fetchWhatsapp = useReduxFetch(sendWhatsapp)
  ...
  const columns = getColumns(whatsappState.send)

  return (
    <>
      <HeaderSection title='Envío masivo whatsapp'>
      </HeaderSection>
      <HeaderFlex>
        <span>Agregar el archivo .csv para enviar whatsapp:</span>
        <br></br>
        <Upload
          accept='.csv'
          name='file'
